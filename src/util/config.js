...
global.daemon_rpc_ws = `wss://${self_hostname}:9830`;
...
function loadConfig(version) {
  try {
    ...
    // 1) ACHI_ROOT environment variable
    // 2) version passed in and determined by the `achi version` call

    // check if ACHI_ROOT is set. it overrides everything else
    const config_root_dir =
      'ACHI_ROOT' in process.env
        ? process.env.ACHI_ROOT
        : path.join(os.homedir(), '.achi', version);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 9830); // jshint ignore:line
