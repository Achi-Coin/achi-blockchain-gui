...
const getAchiVersion = () => {
  let version = null;
  const exePath = getExecutablePath('achi');
  // first see if we can get a achi exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or achi is on the path
    try {
      version = child_process
