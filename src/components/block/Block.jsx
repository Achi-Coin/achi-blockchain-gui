...
import { Card, FormatLargeNumber, Link, Loading, TooltipIcon, Flex } from '@achi/core';
import {
  unix_to_short_date,
  hex_to_array,
  arr_to_hex,
...
export default function Block() {
  ...
  const poolReward = sten_to_achi(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = sten_to_achi(calculateBaseFarmerReward(blockRecord.height));

  const achiFees = blockRecord.fees
    ? sten_to_achi(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: achiFees ? `${achiFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    >
      <Card
        title={(
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Achi
              blockchain
            </Trans>
          </BlockTitle>
        )}
