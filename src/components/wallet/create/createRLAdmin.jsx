...
import { AlertDialog } from '@achi/core';
import {
  Typography,
  Button,
  Box,
...
import { achi_to_sten } from '../../../util/achi';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      achiper_input.value === '' ||
      Number(achiper_input.value) === 0 ||
      !Number(achiper_input.value) ||
      isNaN(Number(achiper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const achiper = achi_to_sten(achiper_input.value);
    const achiper_value = Number.parseInt(Number(achiper));
    ...
    // var fee = achi_to_sten(fee_input.value);
