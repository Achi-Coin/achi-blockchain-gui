...
import { AlertDialog, Card, Flex } from '@achi/core';
...
import {
  sten_to_colouredcoin_string,
  colouredcoin_to_sten,
} from '../../../util/achi';
      ...
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {sten_to_colouredcoin_string(props.balance)} {cc_unit}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
function BalanceCard(props: BalanceCardProps) {
  ...
  const balance_ptotal_achi = sten_to_colouredcoin_string(balance_ptotal);
  const balance_pending_achi = sten_to_colouredcoin_string(balance_pending);
  const balance_change_achi = sten_to_colouredcoin_string(balance_change);
  const acc_content =
    balancebox_1 +
    balancebox_2 +
    balancebox_ptotal +
...
function SendCard(props: SendCardProps) {
  ...
  function send() {
    ...
    const amount = colouredcoin_to_sten(amount_input.value);
    const fee = colouredcoin_to_sten(fee_input.value);

    if (address.includes('achi_addr') || address.includes('colour_desc')) {
      dispatch(
        openDialog(
          <AlertDialog>
