...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const hexacheck = /[\da-f]+$/gi;

    if (!hexacheck.test(ip_val) || ip_val.value === '') {
    ...
    const achiper_input = ip_parsed.limit;
    ...
    const achiper_value = Number.parseInt(Number(achiper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (achi per interval):{' '}
                  {sten_to_achi_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
