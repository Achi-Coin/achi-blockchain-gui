...
import achiEnvironment from '../util/achiEnvironment';
import achiConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!achiEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    achiConfig.loadConfig(achiEnvironment.getAchiVersion());
    ...
    const createWindow = () => {
      ...
      if (!achiConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !achiConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (achiConfig.manageDaemonLifetime()) {
        achiEnvironment.startAchiDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Achipedia' }),
            click: () => {
              openExternal(
                'https://achicoin.org/achipedia/',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Achi-Coin/achi-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Achi-Coin/achi-blockchain/',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Achi-Coin/achi-blockchain/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://twitter.com/Achi_Coin');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Achi Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Achi' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Achi Blockchain' }),
            click: () => {
              openAbout();
            },
          },
